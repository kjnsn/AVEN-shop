...
scheduledBanner={isBetweenDates:function(startDate,endDate){var currentDate=new Date();var startDate=Date.parseString(startDate,"M/d/yyyy H:mm:ss").setTimezone("PST");var endDate=Date.parseString(endDate,"M/d/yyyy H:mm:ss").setTimezone("PST");return(currentDate.isAfter(startDate)&&currentDate.isBefore(endDate));},show:function(scheduleInfo,defaultSelector){var showDefault=true;for(var i=0;i<scheduleInfo.length;i++){scheduledElement=scheduleInfo[i];if(scheduledBanner.isBetweenDates(scheduledElement.startDate,scheduledElement.endDate)){$(scheduledElement.elementSelector).show();showDefault=false;break;}}if(typeof defaultSelector!="undefined"){if(showDefault){$(defaultSelector).show();}}}};
var popUpWin = false;  
